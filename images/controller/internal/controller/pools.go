...
// are deleted (drift correction). Returns (done, error); the upsert+prune
// pair completes in a single reconcile pass so done is always true unless
// err is set.
func (r *SdsElasticClusterReconciler) ensureBlockPools(ctx context.Context, cluster *v1alpha1.SdsElasticCluster) (bool, error) {
...
func (r *SdsElasticClusterReconciler) ensureFilesystems(ctx context.Context, cluster *v1alpha1.SdsElasticCluster) (bool, error) {
	desiredNames := map[string]struct{}{}
...
func (r *SdsElasticClusterReconciler) ensureObjectStores(ctx context.Context, cluster *v1alpha1.SdsElasticCluster) (bool, error) {
	desiredNames := map[string]struct{}{}
...
func (r *SdsElasticClusterReconciler) pruneOwnedByGVK(ctx context.Context, gvk schema.GroupVersionKind, clusterName, namespace string, desired map[string]struct{}) error {
	...
	for i := range list.Items {
		...
		if err := r.deleteUnstructuredIfExists(ctx, gvk, item.GetNamespace(), item.GetName()); err != nil {
