...
// rook-ceph-mon secret (data.fsid) and the CephCluster status.ceph.fsid.
// Either is acceptable: Rook populates them around the same time.
func (r *SdsElasticClusterReconciler) readCephFSID(ctx context.Context) (string, error) {
	if fsid := r.readRookSecretFSID(ctx); fsid != "" {
		return fsid, nil
	}
