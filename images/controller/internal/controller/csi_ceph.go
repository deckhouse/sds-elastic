...
// readRookSecretFSID returns the cluster FSID stored in the rook-ceph-mon
// Secret, or "" if the Secret (or the fsid key) is not yet present. This
// is a narrower variant of readRookMon used by readCephFSID, which does
// not need monitors/credentials.
func (r *SdsElasticClusterReconciler) readRookSecretFSID(ctx context.Context) string {
	secret := &corev1.Secret{}
	if err := r.Client.Get(ctx, types.NamespacedName{
		Namespace: r.Cfg.ControllerNamespace,
		Name:      external.RookCephMonSecretName,
		...
		if !apierrors.IsNotFound(err) {
			r.Log.Error(err, "[readRookSecretFSID] unable to get rook-ceph-mon secret")
		}
		return ""
	}
