...
func CephCluster(cluster *v1alpha1.SdsElasticCluster, namespace, osdStorageClassName, cephImage string) *unstructured.Unstructured {
	...
	if placement := buildPlacement(cluster.Spec.Placement); placement != nil {
...
func buildStorage(cluster *v1alpha1.SdsElasticCluster, osdStorageClassName string) map[string]interface{} {
	...
	if cluster.Spec.Storage.Devices != nil {
		...
		// An explicit node list always implies useAllNodes=false on Rook,
		// regardless of the CR's top-level UseAllNodes default.
		if len(d.Nodes) > 0 {
		...
		if nodes := buildStorageNodes(d.Nodes); len(nodes) > 0 {
			storage["nodes"] = nodes
		}
		return storage
	}
...
// buildStorageNodes converts the user-supplied per-node device overrides into
// the shape Rook expects under spec.storage.nodes[].
func buildStorageNodes(nodes []v1alpha1.DeviceNodeSpec) []interface{} {
	out := make([]interface{}, 0, len(nodes))
	for _, n := range nodes {
		entry := map[string]interface{}{"name": n.Name}
		if n.UseAllDevices != nil {
			entry["useAllDevices"] = *n.UseAllDevices
		}
		if n.DeviceFilter != "" {
			// Same heuristic as the top-level filter so per-node overrides
			// can use either short-name or full-path regexes.
			if isDevicePathRegex(n.DeviceFilter) {
				entry["devicePathFilter"] = n.DeviceFilter
			} else {
				entry["deviceFilter"] = n.DeviceFilter
			}
		}
