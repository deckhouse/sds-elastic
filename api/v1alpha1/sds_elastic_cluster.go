...
type SdsElasticClusterSpec struct {
	...
	// Placement pins Ceph daemon pods (mon, mgr, osd, mds, rgw, cleanup,
	// arbiter, and the "all" catch-all) to specific nodes and lets them
	// tolerate taints. Mapped directly into the Rook CephCluster
	// spec.placement map.
	// +optional
...
type DevicesStorageSpec struct {
	// UseAllNodes selects every node for OSD placement. Ignored when
	// Nodes is non-empty (an explicit node list always implies
	// useAllNodes=false in the resulting Rook spec).
	...
	// Nodes restricts OSD provisioning to the listed hostnames
	// (kubernetes.io/hostname). When set, the controller forces
	// useAllNodes=false on Rook and emits spec.storage.nodes[] with
	// per-node overrides taken from the entry; fields not specified
	// per node fall back to the top-level Devices defaults.
	// +optional
	Nodes []DeviceNodeSpec `json:"nodes,omitempty"`
}

// +k8s:deepcopy-gen=true
type DeviceNodeSpec struct {
	// Name is the Kubernetes hostname of the storage node, exactly
	// as reported by .metadata.name on the Node object (also matches
	// the kubernetes.io/hostname label value).
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MinLength=1
	Name string `json:"name"`

	// UseAllDevices, when set, overrides Devices.UseAllDevices for
	// this node only.
	// +optional
	UseAllDevices *bool `json:"useAllDevices,omitempty"`

	// DeviceFilter overrides Devices.DeviceFilter for this node only.
	// Same auto-routing between deviceFilter / devicePathFilter as
	// the top-level field.
	// +optional
	DeviceFilter string `json:"deviceFilter,omitempty"`
}
...
// +k8s:deepcopy-gen=true
// PlacementSpec groups Ceph daemon placement directives. Field names match
// Rook's spec.placement keys (https://rook.io/docs/rook/latest-release/CRDs/Cluster/ceph-cluster-crd/#placement-configuration-settings).
// Daemons not present here inherit "all".
type PlacementSpec struct {
	// All applies to every Ceph daemon (mon, mgr, osd, ...). Daemon
	// specific entries below add to it.
	// +optional
	All *PlacementEntry `json:"all,omitempty"`
	// +optional
	Mon *PlacementEntry `json:"mon,omitempty"`
	// +optional
	Mgr *PlacementEntry `json:"mgr,omitempty"`
	// +optional
	OSD *PlacementEntry `json:"osd,omitempty"`
	// +optional
	MDS *PlacementEntry `json:"mds,omitempty"`
	// +optional
	RGW *PlacementEntry `json:"rgw,omitempty"`
	// +optional
	Cleanup *PlacementEntry `json:"cleanup,omitempty"`
	// +optional
	Arbiter *PlacementEntry `json:"arbiter,omitempty"`
}
