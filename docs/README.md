...
- The vendored Rook operator registers its CRs under the renamed API group `internal.ceph.rook.io` (upstream uses `ceph.rook.io`); this isolates sds-elastic from a user-installed Rook on the same cluster.
- Direct edits of Rook (`internal.ceph.rook.io`) and ObjectBucket (`objectbucket.io`) resources in the `d8-sds-elastic` namespace are rejected by a validating webhook. All changes must go through `ElasticCluster` / `ElasticStorageClass`.
