...
> **И `public`, и `cluster` должны быть хостовыми подсетями, а не сетью подов.** Модуль поднимает Ceph с `network.provider: host`, поэтому каждый демон (mon/mgr/osd) биндится напрямую на хостовый интерфейс и обязан иметь IP из одного из указанных CIDR. Указание pod-подсети в любом из полей недопустимо — у демонов нет интерфейса в этом диапазоне, и они не смогут забиндиться. Эти поля — это *не* «где живут клиенты» против «где живут поды Ceph»; обе сети — это сети, на которых сидят сами демоны Ceph:
>
> - **`public`** — клиентский трафик (поды Kubernetes обращаются к mon/osd через csi-ceph) плюс весь межслужебный трафик Ceph, кроме репликации OSD.
> - **`cluster`** — только бэкенд OSD↔OSD: репликация, backfill, recovery и heartbeat.
>
> Каждое поле — это список, поэтому можно передать несколько CIDR, если storage-узлы расположены более чем в одной хостовой подсети:
>
> ```yaml
>   network:
>     public:
...
> ```
>
> Если есть выделенный storage-NIC — используйте его подсеть для `cluster`; иначе задайте `cluster` теми же CIDR, что и `public` (как в примере выше). Клиенты должны иметь маршрут в подсети `public`.
