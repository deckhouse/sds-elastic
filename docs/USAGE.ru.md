...
>
> Если есть выделенный storage-NIC — используйте его подсеть для `cluster`; иначе задайте `cluster` тем же CIDR, что и `public` (как в примере выше). Клиенты должны иметь маршрут в подсеть `public`.
